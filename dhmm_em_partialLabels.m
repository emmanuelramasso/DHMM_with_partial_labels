function [LL, prior, transmat, obsmat, nrIterations] = ...
   dhmm_em_partialLabels(data, prior, transmat, obsmat, pl, varargin)
%%%%%%%%%%%%%ù
% This file is from the BayesNet Toolbox of K.P. Murphy
% modified to take into account partial labels on hidden states
%
...
[max_iter, thresh, verbose, obs_prior_weight, adj_prior, adj_trans, adj_obs] = ...
   process_options_2(varargin, 'max_iter', 10, 'thresh', 1e-4, 'verbose', 1, ...
...
while (num_iter <= max_iter) & ~converged
   ...
   [loglik, exp_num_trans, exp_num_visits1, exp_num_emit] = ...
      compute_ess_dhmm(prior, transmat, obsmat, data, obs_prior_weight, pl);
...
function [loglik, exp_num_trans, exp_num_visits1, exp_num_emit, exp_num_visitsT] = ...
   compute_ess_dhmm(startprob, transmat, obsmat, data, dirichlet, pl)
...
for ex=1:numex
   ...
   if 0
      disp('N''UTILISE PAS LES TRANSITIONS');
      transmat(:,:)=1;
   ...
   obslik = obslik .* plau;
   [alpha, beta, gamma, current_ll, xi_summed] = ...
